...
function play() {
  ...
  let tala = 0;
  while(tala!=random) {
    tala = prompt("Giskaðu á tölu á milli 1 og 100");
    if(tala===null) {
      gisk = 0;
      break;
    }
    tala = parseGuess(tala);
    alert(getResponse(tala,random));
    gisk++;
  } 
  games[games.length]=gisk;
  //TODO: ekki vista ágiskanir ef hætt er í leik þegar beðið er um ágiskun
  //Láta leikinn hætta þegar ýtt er á cancel
...
function getResults(){
  ...
  else return `Þú spilaðir ${games.length} leiki \nMeðalfjöldi ágiskana var ${calculateAverage()}`;
}
...
function calculateAverage(){
  const fjLeikja = games.length;
  let fjGisk = 0;
  for(let i=0; i<fjLeikja; i++) {
    fjGisk+=games[i];
  }
  return fjGisk/fjLeikja;
}
...
function getResponse(guess, correct){
  let munur = Math.abs(correct - guess);
  if(guess < 0 || guess===NaN) {
    return 'Ekki rétt';
  }
  if(guess === correct) return 'Rétt';
  if(munur < 5) return 'Mjög nálægt';
  if(munur < 10) return 'Nálægt';
  if(munur < 20) return 'Frekar langt frá';
  if(munur < 50) return 'Langt frá';
